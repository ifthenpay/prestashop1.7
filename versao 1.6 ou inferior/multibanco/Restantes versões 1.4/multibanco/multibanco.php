...
class Multibanco extends PaymentModule
{
	...
	*/
			...
			if ($result AND isset($order->id))
			{
				...
				$ref = $this->GenerateMbRef(Configuration::get('MULTIBANCO_ENT_ID'),Configuration::get('MULTIBANCO_SUBENT_ID'),(int)($order->id),$order->total_paid);
					...
					if (Validate::isEmail($customer->email))
						Mail::Send((int)$order->id_lang, 'order_conf_mb', Mail::l('Order confirmation', (int)$order->id_lang), $data, $customer->email, $customer->firstname.' '.$customer->lastname, NULL, NULL, $fileAttachment);
				}
	...
	/* 
	 * Fun��o para tratamento do callback
     */	 

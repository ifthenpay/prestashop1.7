...
class IfthenpaygatewayOrderDetail extends IfthenpaygatewayBase implements OrderDetailInterface
{
    public function setSmartyVariables()
    {
		...
		$this->smartyDefaultData->setStatus(!empty($this->paymentDataFromDb) ? $this->paymentDataFromDb['status'] : '');

        $this->smartyDefaultData->setPaymentUrl(!empty($this->paymentDataFromDb) ? $this->paymentDataFromDb['payment_url'] : '');

		$deadline = !empty($this->paymentDataFromDb) ? $this->paymentDataFromDb['deadline'] : '';

        $this->smartyDefaultData->setDeadline($deadline != '' ? (new \DateTime($deadline))->format('d-m-Y') : '');
    }

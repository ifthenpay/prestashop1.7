...
class Ifthenpay extends PaymentModule
{
	...
	public function install()
	{
		...
		if (
			...
			!$this->registerHook('actionAdminControllerSetMedia') || !$this->registerHook('actionFrontControllerSetMedia') || !$this->registerHook('displayBackOfficeHeader') || !$this->registerHook('actionOrderStatusPostUpdate')
		) {
			return false;
		}
	...
	private function isCountryValid($params, $paymentMethod)
	{
		...
		$shippingAddressCode = $params['cart']->id_address_delivery;
		$query = DatabaseFactory::buildDbQuery();
		$query->select('id_country');
		$query->from('address');
		$query->where('id_address = ' . $shippingAddressCode);
		$shippingAddressCountry = \Db::getInstance()->getValue($query);

		return in_array($shippingAddressCountry, $allowedCountries);
	}
	...
	public function hookActionOrderStatusPostUpdate($params)
	{
		...
		if ($order->module != 'ifthenpay' && isset($params['newOrderStatus']) && isset($params['newOrderStatus']->paid) && !$params['newOrderStatus']->paid) {
			return;
		}
